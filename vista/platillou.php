<?php
require_once("layouts/header.php");
?>
<figure class="text-center"><h1 class="display-4">Platillo</h1></figure>
<form action="" method="get" class="form-control">
<?php
    foreach($dato as $key => $value):
        foreach($value as $v):
        ?>

    <label for="txtNom" class="form-label">Nombre:</label>
    <input type="text" id="txtNom" name="nom" class="form-control" value="<?php echo $v['nombre']; ?>"/>

    <label for="txtDes" class="form-label">Descripción:</label>
    <input type="text" id="txtDes" name="desc" class="form-control" value="<?php echo $v['descripcion']; ?>"/>

    <label for="txtCos" class="form-label">Costo:</label>
    <input type="number" id="txtCos" name="cos" class="form-control" step="any" value="<?php echo $v['costo']; ?>"/>

    <label for="txtidT" class="form-label">Id Tipo Platillo:</label>
    <input type="number" id="txtidT" name="idTi" class="form-control" value="<?php echo $v['idTipoPlatillo']; ?>"/>

</br>

<input type="hidden" name="idPlatillo" value="<?php echo $v['idPlatillo']; ?>"/></br>
<div class="d-grid gasp-2">
        <input type="submit" class="btn btn-primary" name="btn" value="Actualizar"> <br>
        </div>
        <input type="hidden" name="m" value="actualizar">
        <?php
        endforeach;
    endforeach;
    ?>
</form>
<?php
require_once("layouts/footer.php");